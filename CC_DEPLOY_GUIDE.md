...
```bash
claude -p "Разверни проект: создай .env из .env.example, запусти docker-compose up -d --build, проверь health endpoint, настрой webhook listener" --dangerously-skip-permissions
```
...
6. Настрой webhook listener как systemd сервис
7. Покажи мне итоговый статус
```
...
### Диагностика проблем:
```
Проверь статус всех сервисов: docker-compose ps, логи каждого контейнера, health endpoint, статус webhook listener
```
